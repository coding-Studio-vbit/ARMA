...
export const AddForums = ({ isEdit }: AddStudentsProps) => {
  ...
  const deleteItem = async () => {
    setShowError("");
    const res = await axios.post(
      process.env.REACT_APP_SERVER_URL + "forum/deleteForum",
      { id: id }
    );
    const data = res.data;
    if (data.status === 1) {
  ...
  const loginValidate = async () => {
    if (
      ...
      if (!isEdit) {
        setShowError("");
        const res = await axios.post(
          process.env.REACT_APP_SERVER_URL + "admin/addForum",
          {
            name: forumID,
            phone: phone,
            facultyCoordinatorID: selectCoord,
            forumHeads: selectHead,
            email: email,
            password: password,
          }
        );
        ...
        const res = await axios.put(
          process.env.REACT_APP_SERVER_URL + "forum/editForum",
          {
            id: id,
            name: forumID,
            phone: phone,
  ...
  useEffect(() => {
    const students = async () => {
      ...
      for (let i = 0; i < data.length; i++) {
        arr.push({
          value: data[i]._id,
          label: data[i].name + "-" + data[i].rollNumber,
        });
      }
      setMyStu(arr);
    };
