...
export const AddHalls = ({isEdit}:AddHallsProps) => {
  ...
  const validateName = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const deleteItem = async() => {
        setShowError("");
        const res = await axiosInstance.post(process.env.REACT_APP_SERVER_URL + "halls/deleteHall", {id:id})
        const data = res.data
        if (data.status === 1) {
          setResponse("Deleted")
          setShow(true)
    ...
    {
        setShowError("");
        const res = await axiosInstance.post(process.env.REACT_APP_SERVER_URL + "halls/addHalls", {name:name, block:block, capacity: quantity, hallInfo: hallInfo,})
        const data = res.data
        if (data.status === 1) {
      ...
      setShowError("");
        const res = await axiosInstance.put(process.env.REACT_APP_SERVER_URL + "halls/editHall", {id:id, name:name, block:block, capacity: quantity, hallInfo: hallInfo,})
        const data = res.data
        if (data.status === 1) {
