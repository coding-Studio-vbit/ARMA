    ...
    data = (e.target.files)
      ...
      const handelAttendance = (e:{target:any}) =>{
        if (e.target.checked){
          ...
          dayWiseAttendance[e.target.name] = dayWiseAttendance[e.target.name].filter((val:any)=>val!==e.target.value)
        }
      ...
      const handelCheckbox =(item:any,indx:number)=>{
          ...
          name = {item.name}
          value =  {displayName}
          onChange={(e)=>handelAttendance(e)}></input>
